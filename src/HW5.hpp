...
class CNDO{
    ...
    // HW5 Function - Function that builds x-matrix
    arma::mat Calculate_XMatrix(vector<Atom>& atoms, arma::mat Alpha_Density, arma::mat Beta_Density){
        arma::mat x(num_AO, num_AO, arma::fill::zeros);

        x = 0.5 + (Alpha_Density + Beta_Density);

        return x;
    }


    // HW5 Function - Function that builds y-matrix
    arma::mat Calculate_YMatrix(vector<Atom>& atoms){
        arma::mat y(atoms.size(), atoms.size(), arma::fill::zeros);

        arma::vec Z(atoms.size());
        for (int i = 0; i < atoms.size(); ++i) {
            Z(i) = atoms[i]._atomic_number;
        }
    ...
    // HW5 Function - Function that puts together the derivatives
    // void Compute_Gradient(arma::mat x_matrix, arma::mat y_matrix, vector<Atom>& atoms){
    //     arma::mat gradient(atoms.size(), 3, arma::fill::zeros);
    ...
    //             for(int mu = 0; mu < num_AO; mu++){
    //                 for(int nu = 0; nu < num_AO; nu++){
    //                     grad += x_matrix(mu, nu) * derivative_overlap(mu, nu, atom, coord); 
    //                 }
    //             }

    //             for(int B = 0; B < atoms.size(); B++){
    //                 if (B != atom) {
    //                 grad += y(atom, B) * derivative_gamma(atom, B, R.row(atom), R.row(B));
    //                 }
    //             }
    //         }
    //     }
...
//     void compute_gradient(const arma::mat& p_alpha, const arma::mat& p_beta, const arma::mat& x,
//      const arma::mat& y, const arma::mat& R, vector<Atom>& atoms) {

//         arma::mat gradient(atoms.size(), 3, arma::fill::zeros);
//         // arma::mat gradient(3, atoms.size(), arma::fill::zeros);
...
//                 // Function to calculate the derivative of the overlap integral
//                 auto derivative_overlap = [&](int mu, int nu, int atom_idx, int coord) {
//                     // Placeholder implementation: replace with actual derivative calculations
//                     return 0.0;  // Example value
//                 };

//                 // Function to calculate the derivative of the gamma matrix
//                 auto derivative_gamma = [&](int A, int B, const Atom& atom_A, const Atom& atom_B) {
//                     // Calculate the distance between atoms A and B
//                     arma::rowvec R_diff = {atom_A._x - atom_B._x, atom_A._y - atom_B._y, atom_A._z - atom_B._z};
//                     double distance = arma::norm(R_diff);

//                     // Derivative of gamma with respect to the specified coordinate
//                     if (distance > 1e-10) { // Avoid division by zero
//                         return -R_diff(coord) / std::pow(distance, 3); // Negative gradient w.r.t. inter-atomic distance
//                     } else {
//                         return 0.0; // Overlapping or very close atoms
//                     }
//                 };

//                 // Function to calculate the derivative of the nuclear repulsion energy
//                 auto derivative_Vnuc = [&](int atom_idx, int coord) {
//                     double grad = 0.0;

//                     // Loop over all atoms
//                     for (int other_atom_idx = 0; other_atom_idx < atoms.size(); ++other_atom_idx) {
//                         if (atom_idx != other_atom_idx) {
//                             arma::rowvec R_A = R.row(atom_idx);  // Position of atom A
//                             arma::rowvec R_B = R.row(other_atom_idx);  // Position of atom B
//                             double Z_A = Z(atom_idx);  // Atomic number of atom A
//                             double Z_B = Z(other_atom_idx);  // Atomic number of atom B

//                             // Compute distance between atoms A and B
//                             arma::rowvec r_diff = R_A - R_B;
//                             double distance = norm(r_diff);

//                             // Compute the nuclear repulsion gradient
//                             grad += (Z_A * Z_B) * r_diff(coord) / std::pow(distance, 3);
//                         }
//                     }
...
//                 // Contribution from x * overlap derivatives
//                 for (int mu = 0; mu < num_AO; ++mu) {
//                     for (int nu = 0; nu < num_AO; ++nu) {
//                         grad += x(mu, nu) * derivative_overlap(mu, nu, atom, coord);
//                     }
//                 }

//                 // Contribution from y * gamma derivatives
//                 for (int B = 0; B < atoms.size(); ++B) {
//                     if (B != atom) {
//                         grad += y(atom, B) * derivative_gamma(atom, B, atoms[atom], atoms[B]);
//                     }
//                 }

//                 // Contribution from nuclear repulsion gradient
//                 grad += derivative_Vnuc(atom, coord);

//                 // Store gradient
//                 gradient(atom, coord) = grad;
//             }
//         }
//         arma::mat transposed_gradient = arma::trans(gradient);
//         cout << "gradient" << endl;
//         cout << transposed_gradient << endl;
// }


    // Function that runs the SCF Cycle & updates relevant matrices
    void SCF_Cycle(arma::mat gamma_mat, vector<Atom>& atoms, arma::mat X, arma::mat Y, arma::mat R){
        ...
        while(!converge){
            ...
            if(abs(AlphaDensityMat - OldAlphaDensityMat).max() < 1e-6 && abs(BetaDensityMat - OldBetaDensityMat).max() < 1e-6){
                ...
                cout << "Nuclear Repulsion Energy is " << -0.5 * energy << " eV." << endl;
                cout << "Electron Energy is " << 2 * energy << " eV." << endl;
                compute_gradient(AlphaDensityMat, BetaDensityMat, X, Y, R, atoms);
    ...
    arma::mat gradient(atoms.size(), 3, arma::fill::zeros);
    arma::mat nuclear_gradient(atoms.size(), 3, arma::fill::zeros);
    arma::mat electron_gradient(atoms.size(), 3, arma::fill::zeros);
    ...
    for (int atom = 0; atom < atoms.size(); ++atom) {
        for (int coord = 0; coord < 3; ++coord) {
            ...
            auto derivative_gamma = [&](int A, int B, const Atom& atom_A, const Atom& atom_B) {
                ...
                // Derivative of gamma with respect to the specified coordinate
                if (distance > 1e-10) {
                    return -R_diff(coord) / std::pow(distance, 3);
                } else {
                    return 0.0;
                }
            };
            ...
            // Compute nuclear gradient part
            nuclear_gradient(atom, coord) = derivative_Vnuc(atom, coord);

            // Compute electron gradient part
            for (int mu = 0; mu < x.n_rows; ++mu) {
                for (int nu = 0; nu < x.n_cols; ++nu) {
                    grad += x(mu, nu) * derivative_overlap(mu, nu, atom, coord);
                }
            }
            ...
            electron_gradient(atom, coord) = grad;

            // Total gradient
            gradient(atom, coord) = nuclear_gradient(atom, coord) + electron_gradient(atom, coord);
        }
    }

    // Print gradients
    cout << "Nuclear Gradient Part:" << endl << arma::trans(nuclear_gradient) << endl;
    cout << "Electron Gradient Part:" << endl << arma::trans(electron_gradient) << endl;
    cout << "Total Gradient:" << endl << arma::trans(gradient) << endl;
}
